...
// define various file storage systems supported 
// for each system set the file-writer to use
// also set which one is currently used (default is 'fileSystem')
var fileStorage = {
  used: 'fileSystem',
  writers: {
    'gridStore': 'writeToGridStore'
  }
};
...
fproto.parseMultipartData = function ( chunk, cback ) {
    ...
    var writeToFileStore = function ( dataPayload, cfg ) {
      ...
      if (fileStorage.used == 'fileSystem') {
        writeToFileStream(dataPayload, cfg);
        
      } else {
        // dynamic call to a writer for a specific file storage system
        // fx writeToGridStore(dataPayload, cfg);
        fileStorage.writers[fileStorage.used](dataPayload, cfg);
