...
fproto.gridStore = {
  ...
  // set to true in order to force formaline to use GridStore instead (see writeToFileStream)
  used: false,
  status: 'closed',
...
fproto.parseMultipartData = function ( chunk, cback ) {
    ...
    var writeToGridStore = function ( dataPayload, cfg ) {
      var gs = me.gridStore.save(cfg.path, cfg.store_options);
      gs.open(function(err,store) {
        me.fileSize = 0;
        // can we even do this on heroku?
        // only used in order to ensure sha1sum is updated correctly (see below)
        ...
        me.logger( 3, '\033[7;37mformaline, a new GridStore file was created --> \033[0;34m', cfg.path, '\033[0m' );
        ...
        if ( fieldName ) {
            ...
            if ( fileName ) { // file field
                ...
                wok = if (me.gridStore.active) {
                  writeToFileStream( null, fileConfig);
                } else {
