...
var fs = require( 'fs' ),
    ...
    formaline = function ( config ) {
        ...
        if( config && ( typeof config === 'object' ) ) {
            // https://github.com/rootslab/formaline/issues/36
            // ensure uploadRootDir always is normalized and always ends with a '/'
            if (config.uploadRootDir) {
                config.uploadRootDir = path.normalize(uploadRootDir) + '/';
            }
...
// requires formaline.gridStore.used = true;
// requires GridStore config object used to setup formaline (see below)

// DB object - new Db('my_mongodb', new Server("127.0.0.1", 27017, options_hash_here);
...
//   gridfs_name: 'abc',
//   options: {},
//   close: function(err, result) - optional close callback
// }
// 
...
fproto.gridStore = {
  save: function(filename, store_options) {
    var me = this;
    if (me.cached_gridstore)
      return cached_gridstore;

    var gs_options = store_options || me.gridstore.options || {}
    me.cached_gridstore = new mongodb.GridStore(me.gridstore.db, filename, "w", gs_options);    
    return me.cached_gridstore;
  },
  // set to true in order to force formaline to use GridStore instead (see writeToFileStore)
  used: false,
  status: 'closed',
  files: function(callback) {
    client.collection(me.gridstore.db, callback);
  }
...
fproto.parse = function ( req, res, next ) {
    ...
    var hs = req.headers,
        ...
        startParsing = function () {
            req.removeAllListeners( 'data' ); // do not move from here!
            ...
            if ( ctype ) {
                ( function () {
                    var onClose = me.closeConnection.bind( me );
                    var onData;
                    var onEnd;
                    if ( isUpload ) {
                        // multipart form data
...
fproto.parseMultipartData = function ( chunk, cback ) {
    ...
    // the cfg object passed in must specify the file attributes 
    // used for the jsonFileProgression object.
    // you can specify cfg.store_options to further control how the file is stored in the gridstore
    var writeToGridStore = function ( dataPayload, cfg ) {
      var gs;
      try {
        if ( dataPayload ) {
          // Write data and automatically close on finished write
          gs.write(dataPayload, function(err, gridStore) {
            file.mtime = new Date(); // it is quite accurate when file data were received in only one chunk
            me.logger( 3, '\033[0;37mformaline, new data were written to this GridStore file  --> \033[0;34m', gs.filename, '\033[0m' );
            ...
            // when file finished writing, close the GridStore connection to this file
            gs.close(function(err, result) {
              me.gridStore.status = 'closed';
              me.emitEvent( 'fileend', dataPayload );
            });                    
          }); // write
        }          
        else {
          if ( cfg && cfg.path ) {
            gs = me.gridStore.save(cfg.path, cfg.store_options);
            gs.open(function(err,store) {
              me.fileStream = new fs.WriteStream( cfg.path );
              me.fileSize = 0;
              var file = me.fileStream;

              me.gridStore.status = 'open';
              me.emitEvent( 'filestart', jsonFileProg(file, me));
              apply( gs, cfg, true );
              // note: watch file makes no sense for gridStore

              me.logger( 3, '\033[7;37mformaline, a new GridStore file was created --> \033[0;34m', me.fileStream.path, '\033[0m' );
            }        
          } // if
        } // else    
      } catch ( fserr ) {
        ...
        me.emitEvent( 'GridStore streamexception', jsonMultiPartErr, 0 );
        return false;
      } // try
      return true;
    }); // function
    ...
    var writeToFileStore = function ( dataPayload, cfg ) {
      // redirect if GridStore is being used
      if (me.gridStore.used == true) {
        writeToGridStore(dataPayload, cfg);
      } else {
        writeToFileStream(dataPayload, cfg);
      }
    }
            ...
            try {         
                if ( dataPayload ) {
                    ...
                    if ( cfg && cfg.path ) {
                        ...
                        if ( ! isWindows ) {
                            if ( fs.watch ) {
                                fs.watch( cfg.path, function ( action, filename ) {
                                    var cpath = cfg.path;
                                    fs.stat( me.uploadRootDir + filename, function ( err, stats ) {
                                        if ( ! err && me.fileStream  && me.fileStream.path == cpath ) {
                                            me.fileStream.mtime = stats.mtime;
                                        }
                                    } );
                                } );
                            }
                            else if ( fs.watchFile ) {
    ...
    if ( me.bytesReceived <= me.uploadThreshold ) { // is size allowed? 
        if ( me.fileStream ) {
                ...
                if ( resultsLength === 0 ) { // chunk is only data payload
                    ...
                    if ( ! checkSize( fileDataChunk ) ) {
                        ...
                        if ( ( fileDataChunk.length > 0 ) && ( me.bytesWrittenToDisk + fileDataChunk.length < me.uploadThreshold ) ) {
                            ...
                            wok = writeToFileStore( fileDataChunk );
                            if ( ! wok || ! cok ) {
                                // TODO
                                addToIncompleteList( me.fileStream );    
                                return; 
            ...
            me.emitEvent( 'missing file target exception', {msg: 'neither fileStream or gridStore have been created'}, 0 );
          }      
        }
    } else {
        if ( !me.maxSizeExceeded ) {
    ...
    for ( var i = 0; i < resultsLength; i++ ) {
        ...
        if ( fieldName ) {
            ...
            if ( fileName ) { // file field
                ...
                wok = if (me.gridStore.active) {
                  writeToFileStore( null, fileConfig);
                } else {
                  // create new fileStream                  
                  writeToGridStore( null, fileConfig);
                }
                if ( ! wok ) {
                    // TODO
                    return; 
